...
[root@offline-almalinux8-194-computing ~]# mv /etc/munge/munge.key /home/munge/munge.key
[root@offline-almalinux8-194-computing ~]# chown munge.munge /home/munge/munge.key
[root@offline-almalinux8-194-computing ~]# ln -sf /home/munge/munge.key /etc/munge
```

## 运行
```bash
[root@offline-almalinux8-194-computing ~]# sudo -u munge munged --key-file=/home/munge/munge.key
[root@offline-almalinux8-194-computing ~]# ps -ef|grep munged
munge       4682       1  0 10:22 ?        00:00:00 munged --key-file=/home/munge/munge.key
```

## 停止
```bash
[root@offline-almalinux8-194-computing ~]# sudo -u munge munged --stop
```

# 非系统下
## 安装
我们使用spack来安装munge，请注意需要将状态文件夹设置到/var或一个本地目录，因为每台机器运行都要维持状态。
```bash
